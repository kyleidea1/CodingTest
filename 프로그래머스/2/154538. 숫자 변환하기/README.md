...
### 요약
처음에는, 빈칸을 모두 0으로 초기화해서,, min 함수를 쓰는 부분에 에러가 있었다!
그래서 무한으로 초기화를 하고, min함수가 더 잘 작동하게끔 바꿨다. 이런 류의 문제는 초기화를 무한으로 하기!
