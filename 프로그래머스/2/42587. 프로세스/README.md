...
### 요약
내가 짜 놓고 참.. 파이써닉한 코드! all, any를 적극적으로 쓰면 참 좋을 것 같다는 생각이 든다.
any(i[1] < item[1] for i in q] <- 이 부분에서, 리스트 컴프리헨션 중 단 하나라도 True가 있으면 True를 주는 any, 그리고 모두가 True여야 True를 주는 all!
잘 쓰면 정말 강력한 도구가 될 것 같다ㅎㅎ
그리고 priorities 사이즈 보니까 100이길래 바로 그냥 단순 구현한 것도 좋았던 포인트~
