...
### 요약
처음에 위상 정렬인가 했으나? 포맷이 다르다. 다른 스킬이 끼어들 수 있기 때문.
또한 n이 그렇게 크지 않아서, 맨 첫 스킬부터 차례차례 찍어나가는 지 체크하는 코드를 작성했다.
런타임 에러는!! l이라는 리스트(스킬 선후관계에 포함된 스킬들의 인덱스리스트..?)에 대해 l이 빈 문자열일 수도 있다는 것을 고려하지 않아서 생긴 에러였다!
